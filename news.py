...
MARKET_TRIGGERS: list[tuple[re.Pattern, str, int]] = [
    (re.compile(r"ключев\w* ставк|банк россии (повыс|сниз|сохран)|цб (повыс|сниз|сохран)|набиуллин", re.I), "ЦБ/ставка", 3),
    (re.compile(r"санкци\w*.{0,80}(росси|рф\b|российск|москв|банк|нефт|газ|сбер|втб|мосбирж|нкц|нрд|spfs|swift)|"
                r"(росси|рф\b|российск|против москвы|евросоюз|ес\b|минфин сша|ofac|\d+-?й пакет).{0,80}санкци", re.I),
     "санкции", 2),
...
def is_relevant(item: NewsItem, mode: str) -> bool:
    ...
    if item.tickers:
        return True
    return mode == "market" and item.priority >= 2
